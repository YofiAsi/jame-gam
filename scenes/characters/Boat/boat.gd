...
func _ready() -> void:
	...
	if job == Job.COIN:
		curr_state = State.TO_COIN
	else:
		curr_state = State.TO_FOOD
...
func awake() -> void:
	curr_state = State.TO_FOOD if job == Job.FOOD else State.TO_COIN 
	self.show()
